...
class SolverODE:
    def __init__(self, equation, T, m0, batch_size, valid_size, n_maxstep):# ---------- TF2
        ...
        self.stdNN =                1.e-2
        self.lr_boundaries =        [500, 1000, 2000] # FOR LRSCHEDULE
        self.lr_values =            [1e-2, 3e-3, 1e-3, 3e-4] # FOR LRSCHEDULE
        self.activation_fn_choice = tf.nn.sigmoid # tf.nn.leaky_relu

        self.alpha_I_pop =          self.equation.lambda3
        self.g =                    self.equation.g
        self.Delta_t =              self.equation.Delta_t
        self.Nmax_iter =            int(math.ceil(self.T/self.Delta_t))
        self.Nstates =              5 # S I R for now
        ...
        def __init__(self, Nstates):#, miniminibatch_size):
            ...
            self.layer4_u0 = tf.keras.layers.Dense(units=50, activation='sigmoid',
                                                   kernel_initializer=tf.keras.initializers.RandomNormal(mean=0.0,
                                                                                                         stddev=0.5),
                                                   bias_initializer='zeros')
            self.layer5_u0 = tf.keras.layers.Dense(units=Nstates, activation ='linear')  # second experiment change 2 (was linear)
    ...
    def forward_pass(self, n_samples):
        ...
        Z_empirical = tf.reshape((X[:, self.Nstates] ** (-self.g)) * np.mean(self.alpha_I_pop * (X[:, self.Nstates] ** (-self.g)) * X[:, 2]), [n_samples, 1])
        # print("Z: ", Z_empirical)
        ALPHA = self.equation.optimal_ALPHA(U, Z_empirical)
        # print(U[1:5, :])
        ...
        P_next = P + self.equation.driver_P(Z_empirical, P, ALPHA) * self.Delta_t
        # print("P_next:", P_next[1, :])
        # print("P:", P[1, :])
        U_next = U + self.equation.driver_U(Z_empirical, U, ALPHA) * self.Delta_t
        ...
        Z_empirical = tf.reshape((X[:, self.Nstates] ** (-self.g)) * np.mean(self.alpha_I_pop * (X[:, self.Nstates] ** (-self.g)) * X[:, 2]), [n_samples, 1])
        # print("id: ", self.id_)
        # print("check for weight: ", ((self.id_[0,0] * self.id_[0,0])**(0.2) * X[0,2] + (self.id_[0,0] * self.id_[0,1])**(0.2) *X[1,2])*0.45)
        # print("Z empirical: ", Z_empirical)

        # print("Z: ", Z_empirical)
            # LOOP IN TIME
        for i_t in range(1, self.Nmax_iter+1):
            if (np.mean(X[:,2]) < 1.e-5):  # TO AVOID REACHING 0 INFECTED AND HAVING NaN
                # print("P_next:", P_next[1, :])
                print("INNER P_empirical[2]<1.e-5 = ", np.mean(X[:,2]) , "\t t = ", t)
                break
            t = t + self.Delta_t
        ...
        # COMPUTE ERROR
        target = np.zeros((1,self.Nstates))
